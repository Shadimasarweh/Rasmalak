export default {
  title: 'Learn',
  subtitle: 'Master your financial future',
  welcome_back: 'Welcome back to your learning journey, {name}!',
  progress_message: "You're making great progress. Continue your lessons to unlock more financial insights tailored for you.",
  continue_learning: 'Continue Learning',
  financial_literacy_score: 'Financial Literacy Score',
  good_standing: 'Good Standing',
  top_users_mena: 'Top {percent}% of users in MENA',
  explore_topics: 'Explore Topics',
  view_all: 'View all',
  lessons: 'Lessons',
  current_learning_path: 'Current Learning Path',
  completed: 'Completed',
  beginner_path: 'Beginner Path',
  module_of: 'Module {current} of {total}',
  financial_foundations: 'Financial Foundations',
  intro_personal_finance: 'Introduction to Personal Finance',
  setting_smart_goals: 'Setting SMART Goals',
  needs_vs_wants: 'Understanding Needs vs. Wants',
  needs_vs_wants_desc: 'Learn how to prioritize spending efficiently.',
  creating_first_budget: 'Creating Your First Budget',
  start: 'Start',
  recommended_for_you: 'Recommended for You',
  based_on_spending: 'Based on your spending',
  spending_recommendation: 'We noticed higher dining out expenses this month. Check out this quick guide on meal prep savings.',
  article: 'Article',
  save_groceries_title: '5 Ways to Save on Groceries',
  save_groceries_desc: 'Learn practical tips to reduce your monthly food bill without sacrificing nutrition.',
  more_learning_paths: 'More Learning Paths',
  view_all_paths: 'View all paths',
  intermediate: 'Intermediate',
  advanced: 'Advanced',
  specialized: 'Specialized',
  investment_basics: 'Investment Basics',
  wealth_building: 'Wealth Building',
  islamic_finance: 'Islamic Finance',
  modules: 'Modules',
  latest_articles: 'Latest Insights',
  budget_rule_title: 'The 50/30/20 Budget Rule Explained',
  emergency_funds_title: 'Emergency Funds: How Much is Enough?',
  compound_interest_title: 'Understanding Compound Interest',
  budgeting: 'Budgeting',
  saving: 'Saving',
  investing: 'Investing',
  min_read: '{min} min read',
  featured_videos: 'Featured Videos',
  budgeting_for_beginners: 'Budgeting for Beginners',
  investing_101: 'Investing 101',
  your_achievements: 'Your Achievements',
  first_steps: 'First Steps',
  first_steps_desc: 'Completed your first lesson',
  budget_master: 'Budget Master',
  budget_master_desc: 'Complete Budgeting 101',
  saver: 'Saver',
  saver_desc: 'Complete Saving module',
  investor: 'Investor',
  investor_desc: 'Complete Investing path',
  money_basics: 'Money Basics',
  budgeting_101: 'Budgeting 101',
  debt_management: 'Debt Management',
  // Mode selectors
  'mode.home': 'Home',
  'mode.paths': 'Learning Paths',
  'mode.articles': 'Insights',
  'mode.videos': 'Videos',
  'mode.topics': 'Topics & Skills',
  'mode.achievements': 'Achievements',
  // Tab switcher (learn page)
  'tab.home': 'Home',
  'tab.articles': 'Insights',
  'tab.videos': 'Videos',
  'tab.topics': 'Topics & Skills',
  'tab.achievements': 'Achievements',
  // Accordion / course cards
  sections_count: '{count} sections',
  continue: 'Continue',
  start_course: 'Start Course',
  courses_count: '{count} courses',
  // Level labels — shared by the course filters, card badges, course hero
  // and the video placeholders
  'level_beginner': 'Beginner',
  'level_intermediate': 'Intermediate',
  'level_advanced': 'Advanced',
  // Course grid (Learn v2)
  'filter_all': 'All courses',
  'filter_in_progress': 'In progress',
  'filter_in_progress_empty': "You haven't started any course yet — pick one to begin.",
  'card_lessons': '{count} lessons',
  'card_minutes': '{min} min',
  'completed_check': 'Completed ✓',
  'review': 'Review',
  'resume_next_lesson': 'Next: {title}',
  // Achievements states
  'achievements_unlocked_of': '{count} of {total} unlocked',
  'achievements_earned': 'Earned ✓',
  'achievements_locked': 'Locked',
  'achievements_unlock_by': 'Unlock by: {condition}',
  'achievements_remaining': '{count} to go',
  // Coming-soon placeholders (Topics & Videos)
  'coming_soon_tag': 'Coming soon',
  'coming_soon_planned': 'Planned',
  'coming_soon_notify': 'Notify me',
  'coming_soon_notified': "You're on the list ✓",
  'coming_soon_toast': "We'll notify you when it launches.",
  'coming_soon_topics_title': 'Topics & Skills are on the way',
  'coming_soon_topics_desc': "Focused skill tracks that connect courses, tools, and practice. These are the tracks we're planning.",
  'coming_soon_videos_title': 'Video lessons are coming',
  'coming_soon_videos_desc': 'Short, practical videos that pair with the courses. Tell us to notify you and be first to watch.',
  // Literacy score milestones
  'milestone.beginner': 'Beginner',
  'milestone.confident': 'Confident',
  'milestone.expert': 'Expert',
  'milestone_path': '{from} → {to} at {score}',
  'milestone_reached': 'Top milestone reached',
  'how_to_raise': 'How to raise it',
  'how_to_raise_explainer': 'Your score is the average completion across all courses. Complete course sections to raise it.',
  // Hero section
  featured_title: 'Start Your Financial Journey',
  featured_desc: 'Begin with our beginner-friendly Financial Foundations path and build your knowledge step by step.',
  get_started: 'Get Started',
  literacy_score: 'Financial Literacy Score',
  score_placeholder: 'Complete lessons to unlock',
  // Content sections
  featured: 'Featured',
  recommended: 'Recommended for you',
  recommendation_text: 'Based on your profile, start with Budgeting 101.',
  recently_added: 'Recently added',
  featured_articles: 'Featured Insights',
  featured_courses: 'Featured Courses',
  current_path: 'Continue Your Path',
  available_paths: 'Available Paths',
  all_articles: 'All Insights',
  all_videos: 'All Videos',
  // Topic card template
  lessons_count: '{count} Lessons',
  modules_count: '{count} Modules',
  // Article categories
  debt: 'Debt',
  debt_snowball_title: 'Debt Snowball vs Avalanche Method',
  // Video titles
  understanding_credit: 'Understanding Credit Scores',
  retirement_planning: 'Retirement Planning Basics',
  // Course viewer
  'course.back_to_courses': 'Back to Learn',
  'course.exit_course': 'Exit Course',
  'course.sections_completed': '{done} of {total} sections completed',
  'course.course_complete': 'Course Complete!',
  'course.not_found': 'Course not found',
  'course.continue': 'Continue',
  'course.start_course': 'Start Course',
  'course.sections_count': '{count} sections',
  'course.available_courses': 'Available Courses',
  'course.next_section': 'Next Section',
  'course.previous_section': 'Previous Section',
  'course.next': 'Next',
  'course.previous': 'Previous',
  'course.complete_course': 'Complete Course',
  // Course viewer stepper
  'course.stepper_section_of': 'Section {current} of {total}',
  'course.stepper_time_left': '~{min} min left',
  'course.hero_lessons_sections': '{lessons} lessons · {sections} sections',
  'course.checkpoint_title': 'Checkpoint',
  'course.toggle_sidebar_open': 'Open sidebar',
  'course.toggle_sidebar_close': 'Close sidebar',
  // Checkpoint quiz
  'course.quiz_answered': '{answered} / {total} answered',
  'course.quiz_correct': 'Correct',
  'course.quiz_incorrect': 'Not quite',
  'course.quiz_passed': 'Checkpoint passed!',
  'course.quiz_passed_sub': 'Section marked complete — keep going.',
  'course.quiz_failed': 'Almost there — review the explanations and try again.',
  'course.quiz_retry': 'Try again',
  'course.quiz_locked_next': 'Pass the checkpoint quiz to continue',
  // Course tutor chat
  'course.tutor_title': 'Course Tutor',
  'course.tutor_placeholder': 'Ask about this course...',
  'course.tutor_minimize': 'Minimize',
  'course.tutor_welcome': 'Need help with the course?',
  'course.tutor_welcome_sub': 'Ask me to explain a topic, expand on a concept, or help you understand anything in this course.',
  'course.tutor_send': 'Send',
  'course.tutor_ask': 'Ask Mustasharak',
  'course.tutor_quick_explain': 'Explain this lesson simply',
  'course.tutor_quick_example': 'Give me a real-life example',
  'course.tutor_quick_summary': 'Summarize the key points',
  'course.tutor_error_generic': 'An error occurred. Please try again.',
  'course.tutor_error_network': 'Connection error. Please check your internet and try again.',
  // Article viewer
  'article.not_found': 'Article not found',
  'article.back_to_learn': 'Back to Learn',
  'article.back_to_articles': 'Back to Insights',
  'articles.heading': 'Practical Financial Insights',
  'articles.subheading': 'Understand your options. Make better money decisions.',
  'article.read_article': 'Read Article',
  // Article reader v2
  'article.on_this_page': 'On this page',
  'article.percent_read': '{percent}% read',
  'article.keep_reading': 'Keep reading',
  'article.author_team': 'Rasmalak Team',
  'articles_count': '{count, plural, one {# article} other {# articles}}',
  // Topics & Skills placeholder tracks
  'topic.budgeting': 'Budgeting',
  'topic.investing': 'Investing',
  'topic.saving': 'Saving',
  'topic.debt_management': 'Debt Management',
  'topic.real_estate': 'Real Estate',
  'topic.stock_market': 'Stock Market',
  'topic.crypto': 'Crypto',
  'topic.insurance': 'Insurance',
  'topic.retirement': 'Retirement',
  'topic.tax_planning': 'Tax Planning',
  'topic.business_finance': 'Business Finance',
  'topic.financial_goals': 'Financial Goals',
  // Video placeholder titles
  'video.stock_markets': 'Introduction to Stock Markets',
  'video.central_banks': 'How Central Banks Work',
  'video.technical_analysis': 'Technical Analysis Basics',
  'video.crypto_blockchain': 'Crypto and Blockchain Explained',
  'video.budgeting_beginners': 'Budgeting for Beginners',
  'video.options_trading': 'Options Trading: Risk and Reward',
  // Achievement badges
  'achievement.first_steps.title': 'First Steps',
  'achievement.first_steps.condition': 'Complete your first lesson',
  'achievement.quick_learner.title': 'Quick Learner',
  'achievement.quick_learner.condition': 'Complete 5 lessons in a week',
  'achievement.consistent.title': 'Consistent',
  'achievement.consistent.condition': '7-day learning streak',
  'achievement.money_basics.title': 'Money Basics',
  'achievement.money_basics.condition': 'Finish Beginner track',
  'achievement.investor_mindset.title': 'Investor Mindset',
  'achievement.investor_mindset.condition': 'Finish Intermediate track',
  'achievement.financial_expert.title': 'Financial Expert',
  'achievement.financial_expert.condition': 'Finish Advanced track',
  'achievement.scholar.title': 'Scholar',
  'achievement.scholar.condition': 'Read 10 articles',
  'achievement.video_student.title': 'Video Student',
  'achievement.video_student.condition': 'Watch 5 videos',
  'achievements_earned_on': 'Earned {date}',
};
