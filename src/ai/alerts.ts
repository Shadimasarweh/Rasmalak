...
export function detectSpendingAlerts(context: UserFinancialContext): SpendingAlert[] {
  ...
  if (context.budget && context.budget.isOverBudget) {
    ...
    alerts.push({
      ...
      message: `You've spent ${fmtNum(overAmount, 'en')} ${currency} over your monthly budget.`,
  ...
  if (context.budget && !context.budget.isOverBudget) {
    ...
    if (percentUsed > expectedPercent + 15 && percentUsed > 60) {
      alerts.push({
        ...
        titleAr: 'صرف سريع',
        message: `You've used ${fmtPct(percentUsed, 'en', 0)} of your budget with ${fmtNum(daysRemaining, 'en')} days left.`,
        messageAr: `صرفت ${fmtPct(percentUsed, 'ar', 0)} من ميزانيتك وباقي ${fmtNum(daysRemaining, 'ar')} يوم.`,
  ...
  if (context.predictive) {
    ...
    alerts.push({
      ...
      title: 'May Run Short',
      titleAr: 'قد لا يكفي',
      message: `At current pace, you may be short ${fmtNum(Math.abs(context.currentMonth.projectedEndBalance), 'en')} ${currency} by month end.`,
      messageAr: `بهالمعدل، ممكن ينقصك ${fmtNum(Math.abs(context.currentMonth.projectedEndBalance), 'ar')} ${currency} آخر الشهر.`,
      actionLabel: 'Plan Ahead',
      actionLabelAr: 'خطط مقدماً',
      actionRoute: '/budget',
      metric: {
  ...
  if (context.predictive) {
    for (const dev of context.predictive.baselineDeviations.slice(0, 2)) {
      ...
      alerts.push({
        ...
        title: `High ${dev.category} Spending`,
        titleAr: `صرف عالي على ${dev.category}`,
        message: `Pacing ${fmtPct(pctAbove, 'en', 0)} above your typical month in ${dev.category}.`,
        messageAr: `وتيرة الصرف ${fmtPct(pctAbove, 'ar', 0)} أعلى من شهرك المعتاد على ${dev.category}.`,
  ...
  if (context.savingsRate < 0.1 && context.netBalance > 0) {
    ...
    if (potentialSavings > 0) {
      alerts.push({
        ...
        title: 'Save More?',
        titleAr: 'وفّر أكثر؟',
      message: `You could save up to ${fmtNum(potentialSavings, 'en')} ${currency} more per month.`,
      messageAr: `ممكن توفر لغاية ${fmtNum(potentialSavings, 'ar')} ${currency} أكثر بالشهر.`,
  ...
  for (const goal of context.goals) {
    if (goal.progressPercentage >= 75 && goal.progressPercentage < 100) {
      alerts.push({
        ...
        title: 'Almost There!',
        titleAr: 'قربت توصل!',
        message: `${fmtPct(goal.progressPercentage, 'en', 0)} towards your "${goal.name}" goal!`,
        messageAr: `${fmtPct(goal.progressPercentage, 'ar', 0)} من هدف "${goal.name}"!`,
...
export function generateGoalSuggestions(context: UserFinancialContext): GoalSuggestion[] {
  ...
  if (!hasEmergencyGoal && context.netBalance > 0) {
    ...
    suggestions.push({
      ...
      description: `Build a safety net of ${fmtNum(emergencyTarget, 'en')} ${currency} (3 months expenses).`,
      descriptionAr: `اعمل شبكة أمان بقيمة ${fmtNum(emergencyTarget, 'ar')} ${currency} (مصاريف ٣ شهور).`,
  ...
  if (context.savingsRate >= 0.15 && context.goals.length < 3) {
    ...
    suggestions.push({
      ...
      titleAr: 'كافئ نفسك',
      description: `You're saving well! Consider setting a fun goal like a vacation (${fmtNum(vacationTarget, 'en')} ${currency}).`,
      descriptionAr: `انت بتوفر منيح! فكر بهدف ممتع مثل سفرة (${fmtNum(vacationTarget, 'ar')} ${currency}).`,
