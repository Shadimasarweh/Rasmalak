...
export class ReportDocument {
  ...
  drawFooter(ctx: PageContext, pageNum: number, totalPages: number): void {
    ...
    const brandLabel = ar ? 'تم إنشاؤه بواسطة رسمالك' : 'Generated by Rasmalak';
    // Build the label in natural reading order and let the shaper handle RTL
    // reordering — hand-swapping the operands here produced «٥ من ١» ("5 of 1").
    const pageNumStr = ar
      ? toArabicNumerals(`صفحة ${pageNum} من ${totalPages}`)
