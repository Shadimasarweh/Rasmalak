      ...
      <div className="hidden lg:flex lg:w-1/2 flex-col justify-between p-10 relative overflow-hidden" style={{ backgroundColor: 'var(--color-sidebar-bg)' }}>
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="relative w-80 h-80">
            ...
            <div className="absolute top-0 left-1/4 w-32 h-32 rounded-[var(--radius-xl)] transform rotate-12 opacity-80" style={{ backgroundColor: 'rgba(255,255,255,0.05)' }} />
            <div className="absolute top-1/4 right-1/4 w-24 h-24 bg-[#8B7355] rounded-[var(--radius-xl)] transform -rotate-6" />
            <div className="absolute bottom-1/4 left-1/3 w-40 h-40 rounded-[var(--radius-xl)] transform rotate-3" style={{ backgroundColor: 'rgba(255,255,255,0.05)' }} />
            <div className="absolute bottom-0 right-1/3 w-28 h-28 rounded-[var(--radius-xl)] transform -rotate-12 opacity-60" style={{ backgroundColor: 'rgba(255,255,255,0.05)' }} />
            {/* Small accent sphere */}
        ...
        <div className="relative z-10">
          <h1 className="text-4xl font-bold text-[#FFFFFF] leading-tight mb-4">
            {intl.formatMessage({ id: 'auth.hero_title', defaultMessage: 'Master your wealth with Intelligent Insights' }).split('Intelligent').map((part, i) => 
              i === 0 ? part : <span key={i}><span style={{ color: 'var(--color-accent-growth)' }}>Intelligent<br />Insights</span></span>
            )}
          </h1>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12" style={{ backgroundColor: 'var(--color-bg-surface-1)' }}>
        <div className="w-full max-w-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            {error && (
              <div className="p-3 rounded-[var(--radius-md)] text-sm" style={{ backgroundColor: 'var(--color-danger-bg)', border: '1px solid var(--color-danger-text)', color: 'var(--color-danger-text)' }}>
                {error}
              </div>
            )}
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-sm" style={{ color: 'var(--color-text-secondary)' }}>{intl.formatMessage({ id: 'auth.remember_for_days', defaultMessage: 'Remember for 30 days' })}</span>
              </label>
              <Link
          ...
          <div className="grid grid-cols-2 gap-3">
            <button
              type="button"
              className="flex items-center justify-center gap-2 px-4 py-3 border rounded-[var(--radius-md)] text-sm font-medium transition-colors hover:opacity-90"
