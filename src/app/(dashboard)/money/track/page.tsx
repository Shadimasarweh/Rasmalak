  ...
  return (
    <div className="ds-page" style={{ direction: isRtl ? 'rtl' : 'ltr' }}>
      ...
      <div className="ds-section-header" style={{ flexWrap: 'wrap', gap: '12px' }}>
        <div>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', color: 'var(--ds-actual)', marginBottom: '4px' }}>
            <span style={{ fontSize: '11px', fontWeight: 600, letterSpacing: '0.04em', textTransform: 'uppercase' }}>
              {intl.formatMessage({ id: 'money.timeline_spend' })}
            </span>
          </div>
          <h1 className="ds-title-page">
            {intl.formatMessage({ id: 'money.track_title' })}
          </h1>
          <p className="ds-body">
            {intl.formatMessage({ id: 'money.track_subtitle' })}
          </p>
          <p
            style={{
              fontSize: '12px',
        ...
        <div className="flex items-center" style={{ gap: 'var(--spacing-1)', flexWrap: 'wrap' }}>
          ...
          >
            <PlusIcon />
            {intl.formatMessage({ id: 'money.track_add_expense' })}
          </Link>
        </div>
      </div>

      {/* OVER-PLAN WARNING — surfaces categories that have already crossed 90% of their planned amount */}
      {hasAnyTransactions && (() => {
        const overPlan = DEFAULT_EXPENSE_CATEGORIES
          .map((cat) => {
            const planned = categoryBudgets[cat.id] || 0;
            const actual = spendingByCategory[cat.id] || 0;
            return { cat, planned, actual };
          })
          .filter((row) => row.planned > 0 && row.actual >= row.planned * 0.9);
        if (overPlan.length === 0) return null;
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {overPlan.slice(0, 3).map(({ cat, planned, actual }) => {
              ...
              return (
                <div
                  ...
                  style={{
                    padding: '10px 14px',
                    background: actual > planned ? 'var(--ds-over-bg)' : 'var(--ds-warning-bg)',
                    border: `0.5px solid ${actual > planned ? 'var(--ds-over-border)' : 'var(--ds-warning-border)'}`,
                    borderRadius: '10px',
                    fontSize: '13px',
                    color: actual > planned ? 'var(--ds-over)' : 'var(--ds-warning-text)',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'space-between',
                    gap: '8px',
                ...
                >
                  <span>
                    {intl.formatMessage(
                      { id: 'money.track_over_plan_warning' },
                      {
                        category: isRtl ? cat.nameAr : cat.name,
                        percent,
                        planned: `${currencySymbol} ${intl.formatNumber(Math.round(planned))}`,
                      },
                    )}
                  </span>
                  <Link href="/money/plan" style={{ color: 'var(--ds-plan)', fontWeight: 600, textDecoration: 'underline', fontSize: '12px' }}>
                    {intl.formatMessage({ id: 'money.track_over_plan_link' })}
                  </Link>
                </div>
              );
            })}
